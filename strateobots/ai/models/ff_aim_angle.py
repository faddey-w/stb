...
angle_base_fields = data.FeatureSet(
    [data.Feature(["orientation"]), data.Feature(["tower_orientation"])]
)
...
coordinates_fields = data.FeatureSet(
    [data.Feature(["x"], coord_scale), data.Feature(["y"], coord_scale)]
)
...
class Model(model_function.TwoStepDataEncoderMixin):
    ...
    def _encode_state(bot, enemy, bot_bullet, enemy_bullet):
        ...
        bot_vector = np.concatenate([bot_visible_fields(bot), bot_private_fields(bot)])
    ...
    def apply(self, state_vector_array):
        ...
        model = KerasModel(
            inputs=[inputs],
            outputs=[
                self.output_blocks[key](state_vector_array) for key in self.control_set
            ],
            name=self.name,
        )
        kwargs = {key: model.output[i] for i, key in enumerate(self.control_set)}
