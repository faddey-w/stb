...
def draw_state(bot, enemy, bot_bullet, enemy_bullet):
    ...
    return np.stack([bot_hp_layer, enemy_hp_layer,
                     bot_gun_layer, enemy_gun_layer,
                     bullets_layer])
...
class Model:
        ...
        with tf.variable_scope(self.name):
            ...
            self.ff = nn.LayerChain(
                nn.Linear.chain_factory(
                    self.out_im_size * self.out_im_size * self.out_layers,
                    'FFBlock'
                ),
    ...
    def apply(self, state_vector_array):
        ...
        image = tf.reshape(state_vector_array, [batch_size, N_LAYERS, IMAGE_DIM, IMAGE_DIM])
        ...
        for i, kernel in enumerate(self.conv_kernels):
            ...
            sz = 3 if i < 2 else 2
            image = tf.nn.pool(image, [sz, sz], strides=[sz, sz],
                               padding='SAME', pooling_type='MAX')
            image_layers.append(image)
        # import pdb; pdb.set_trace()

        x = tf.reshape(image, [batch_size, self.out_layers * self.out_im_size * self.out_im_size])
...
def _draw_bot(hp_image, gun_image, bot):
    ...
    ori_r = 10 + velocity
    _draw_gradient_line_np(hp_image, x, y, 1,
                           x + ori_r * cos(angle), y + ori_r * sin(angle), 1,
                           5)
    _draw_circle_np(hp_image, x, y, radius, 0.1 + 0.9 * bot['hp'])

    angle = bot['orientation'] + bot['tower_orientation']
    _draw_gradient_line_np(gun_image, x, y, 1,
                           x + shot_range * cos(angle), y + shot_range * sin(angle), 1,
                           5)
...
def _draw_bullet(image, bullet):
    ...
    _draw_gradient_line_np(image, x, y, 1,
                           x + r * cos(angle),
                           y + r * sin(angle), 1,
                           5)
