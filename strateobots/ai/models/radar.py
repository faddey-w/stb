...
class Model:
    ...
    state_dimension = (
        _angle_readings.dimension
        + _range_readings.dimension
        + _scalar_readings.dimension
    )
    name = "RadarModel"
    ...
    def __init__(self, name=None):
        ...
        with tf.variable_scope(self.name):
            ...
            self.ff_block = nn.LayerChain(
                nn.Linear.chain_factory(out_im_dim * 12 + scalars_dim, "FFBlock"),
                # (50, tf.sigmoid),
    ...
    def data_encoder(bot, enemy, bot_bullet, enemy_bullet):
        ...
        all_state = dict(bot=bot, enemy=enemy, engineered=extra)
    ...
    def apply(self, state_vector_array):
        ...
        angle_readings = tf.reshape(
            angle_readings, [tf.shape(angle_readings)[0], POINT_DIM, IMAGE_DIM]
        )
        ...
        return self._Apply(
            state_vector_array, scan_nodes, ffmain, move, rotate, tower_rotate, action
        )
...
class Conv1d:
    def __init__(
        self, name, filter_size, in_dim, out_dim, paddind="CIRCULAR", activation=None
    ):
        ...
        with tf.variable_scope(self.name):
            self.kernel = tf.get_variable(
                "Kernel", shape=[filter_size, in_dim + 1, out_dim]
            )
