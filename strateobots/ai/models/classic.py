...
coordinates_fields = data.FeatureSet(
    [data.Feature(["x"], COORDINATE_SCALE), data.Feature(["y"], COORDINATE_SCALE)]
)
...
class Model:
    ...
    _current_state_dimension = sum(
        (angle_features.dimension, range_features.dimension, other_features.dimension)
    )
        ...
        with tf.variable_scope(self.name):
            ...
            self.rotate_block = nn.LayerChain(
                nn.Residual.chain_factory(
                    self.state_dimension, "RotateBlock", allow_skip_transform=True
                ),
                (20, tf.square),
                (20, tf.nn.leaky_relu),
                (20, tf.nn.leaky_relu),
    ...
    def apply(self, state_vector_array):
        ...
        return self._Apply(state_vector_array, move, rotate, tower_rotate, fire, shield)
