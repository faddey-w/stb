...
class RWRTraining:
    def __init__(self, session, storage_directory, logs_dir=None):
        ...
        self.model = ff_aim_angle.Model("RWRModel")
        ...
        self.rwr = rwr = RewardWeightedRegression(
            self.model, batch_size=500, entropy_coeff=0.1
        )
        ...
        if logs_dir is not None:
            actions_entropy = [
                tf.summary.scalar("Entropy/{}".format(ctl), entropy)
    ...
    def run_game(self, opponent):
        ...
        with util.interrupt_atomic():
            return self.replay_memory.add_replay(
                metadata, engine.replay, load_predicate=load_predicate
            )
    ...
    def train_once(self, n_batches):
        ...
        for i in range(n_batches):
            summary = self.rwr.do_train_step(
                self.sess, self.replay_memory, i, self.train_summary_op
            )
        ...
        while True:
            ...
            try:
                ...
                perf_summary.value.add(
                    tag="Perf/Length", simple_value=float(len(rd.json_data))
                )
                perf_summary.value.add(
                    tag="Perf/BotHP", simple_value=float(_get_hp(rd, rd.team, -1))
                )
                perf_summary.value.add(
                    tag="Perf/EnemyHP",
                ...
                perf_summary.value.add(
                    tag="Perf/Reward", simple_value=float(np.mean(reward_function(rd)))
                )
