...
class BaseActionInferenceModelV2:
    ...
    def __init__(self, move, rotate, tower_rotate, fire, shield, common=None):
        self.name = "AIN"
        ...
        with tf.variable_scope(self.name):
            ...
            self.rotate_last = layers.Linear(
                "rotate_last", self.rotate_net.n_features, 3
            )
            self.tower_rotate_last = layers.Linear(
                "tower_rotate_last", self.tower_rotate_net.n_features, 3
            )
            self.fire_last = layers.Linear("fire_last", self.fire_net.n_features, 2)
            self.shield_last = layers.Linear(
                "shield_last", self.shield_net.n_features, 2
            )
    ...
    def apply(self, state, with_exploration=False):
        ...
        if with_exploration:
            inference.explore_move = tf.placeholder(
                tf.float32, [self.move_net.n_features] * 2
            )
            inference.explore_rotate = tf.placeholder(
                tf.float32, [self.rotate_net.n_features] * 2
            )
            inference.explore_tower_rotate = tf.placeholder(
                tf.float32, [self.tower_rotate_net.n_features] * 2
            )
            inference.explore_fire = tf.placeholder(
                tf.float32, [self.fire_net.n_features] * 2
            )
            inference.explore_shield = tf.placeholder(
                tf.float32, [self.shield_net.n_features] * 2
            )
            ...
            def apply_explore(control_name):
                ...
                control_node.features = layers.batch_matmul(
                    control_node.features, explore_mat
                )
        ...
        inference.classify_nodes = dict(
            ...
            tower_rotate=self.tower_rotate_last.apply(
                inference.tower_rotate.features, classify
            ),
    ...
    def generate_exploration_feed(self, inference, strength):
        return {
            getattr(inference, "explore_" + name): strength
            * np.random.standard_normal([getattr(self, name + "_net").n_features] * 2)
