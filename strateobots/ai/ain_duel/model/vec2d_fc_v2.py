...
from strateobots.ai.ain_duel.model.base import (
    normalize_state,
    finite_assert,
    combine_predictions,
)
...
class ActionInferenceModel:
    ...
    def __init__(
        ...
        with tf.variable_scope(self.name):
            ...
            self.tower_rotate_net = Vec2dV2Network(
                "tower_rotate", tower_rotate_vec, tower_rotate_fc
            )
            self.fire_net = Vec2dV2Network("fire", fire_vec, fire_fc)
            self.shield_net = Vec2dV2Network("shield", shield_vec, shield_fc)

            self.move_last = layers.Linear(
                "move_last", self.move_net.ff_chain.out_size, 3
            )
            self.rotate_last = layers.Linear(
                "rotate_last", self.rotate_net.ff_chain.out_size, 3
            )
            self.tower_rotate_last = layers.Linear(
                "tower_rotate_last", self.tower_rotate_net.ff_chain.out_size, 3
            )
            self.fire_last = layers.Linear(
                "fire_last", self.fire_net.ff_chain.out_size, 1
            )
            self.shield_last = layers.Linear(
                "shield_last", self.shield_net.ff_chain.out_size, 1
            )
    ...
    def apply(self, state):
        ...
        inference.action_prediction = combine_predictions(
            ...
            rotate=self.rotate_last.apply(
                self.rotate_net.apply(state)[-1], classify
            ).out,
            tower_rotate=self.tower_rotate_last.apply(
                self.tower_rotate_net.apply(state)[-1], classify
            ).out,
            fire=self.fire_last.apply(self.fire_net.apply(state)[-1], tf.sigmoid).out,
            shield=self.shield_last.apply(
                self.shield_net.apply(state)[-1], tf.sigmoid
            ).out,
        )
        return inference
