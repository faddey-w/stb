...
def train():
    ...
    rl = core.ReinforcementLearning(
        modelset=mdl, batch_size=batch_size, reward_prediction=0.97
    )
    ...
    try:
        ...
        for i in range(n_games):
            ...
            runner.run_one_game(
                replay_memory=mem, ai1_func=ai1_function, ai2_func=ai2_function
            )
        mem.save(data_dir)
    ...
    for i in range(2501):
        [loss, sumry] = rl.do_train_step(
            sess, mem, n_rnd_entries=batch_size, extra_tensors=[rl.loss, summary_op]
        )
