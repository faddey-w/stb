...
class SelectOneAction:
    def __init__(self, qfunc_model, state):
        ...
        all_actions = [
            [1.0 if i == j else 0.0 for j in range(n_actions)] for i in range(n_actions)
        ]

        batch_shape = layers.shape_to_list(state.shape[:-1])
        batched_all_actions = np.reshape(
            all_actions, [n_actions] + [1] * len(batch_shape) + [n_actions]
        ...
        self.action = tf.gather_nd(self.all_actions, tf.expand_dims(self.max_idx, -1))
