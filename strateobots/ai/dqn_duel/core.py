...
class ReinforcementLearning:
    def __init__(
        ...
        self.state_ph = tf.placeholder(
            tf.float32, [batch_size, state2vec.vector_length]
        )
        self.next_state_ph = tf.placeholder(
            tf.float32, [batch_size, state2vec.vector_length]
        )
        self.action_ph = tf.placeholder(
            tf.float32, [batch_size, action2vec.vector_length]
        )
        ...
        if regularization_weight is not None:
            regularization_losses = [
                tf.reduce_mean(tf.square(v)) for v in modelset.var_list
            ]
            self.regularization_loss = tf.add_n(regularization_losses) / len(
                regularization_losses
            )
        ...
        self.train_step = self.optimizer.minimize(
            self.total_loss, var_list=modelset.var_list
        )

        self.init_op = tf.variables_initializer(self.optimizer.variables())

    def do_train_step(
        self, session, replay_memory, extra_tensors=(), **sampling_kwargs
    ):
        _, extra_results = self.compute_on_sample(
            session, replay_memory, [self.train_step, extra_tensors], **sampling_kwargs
        )
    ...
    def compute_on_sample(
        ...
        reward_sample = compute_reward_from_vectors(
            states_before_sample, actions_sample, states_after_sample
        )
