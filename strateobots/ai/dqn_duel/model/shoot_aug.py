...
class QualityFunction:
    def __init__(self, model, state, action):
        ...
        ) - select_features(action, action2vec, "rotate_right", "tower_rotate_right")
        shot_ready = select_features(state, state2vec, (0, "load")) > 0.999

        lin0 = select_features(  # 3
            state, state2vec, (1, "hp_ratio"), (0, "load"), (2, "remaining_range")
        )
        ...
        self.lin_o = model.lin_o.apply(
            tf.concat([self.linear0, self.lin_h.out], -1), tf.identity
        )

        self.log_h = model.log_h.apply(self.linear0, tf.nn.relu)
        self.log_o = model.log_o.apply(
            tf.concat([self.linear0, self.log_h.out], -1), tf.sigmoid
        )
        # import pdb; pdb.set_trace()

        flags = tf.concat(
            [self.log_o.out, self.logical0, 1 - self.log_o.out, 1 - self.logical0], -1
        )
        ...
        self.components = tf.concat(
            [hp * tf.exp(-self.evt_hp.out), tf.exp(-self.evt_const.out)], -1
        )
