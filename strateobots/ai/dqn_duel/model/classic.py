...
class ClassicModel:
    def __init__(self, n_actions, layer_sizes, angle_sections):
        ...
        self.var_list.extend(var for node in self.layers for var in node.var_list)
...
class QualityFunction:
    def __init__(self, model, state, action):
        ...
        partial_input_tensor = tf.concat(
            [ranges_tensor, angles_tensor, cosines, *map(f, OTHER_FEATURES)], -1
        )
        self.input_tensor = tf.concat(
            [partial_input_tensor + tf.zeros_like(action[..., :1]), action], -1
        )
